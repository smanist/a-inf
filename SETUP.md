...
- `.a-inf/config.toml`
- `_sources/` as the local-only source archive, populated during ingest
...
The generated `.env` defaults `QMD_WIKI_COLLECTION` and `QMD_PAPERS_COLLECTION` to the initialized repo directory name, enables source archiving under `_sources/`, and sets source-detail query retrieval to `auto`. Init also creates that QMD collection and runs the first `qmd update` / `qmd embed`.
